---
Type: Doc
Use: Readme docs for Tokyo Terminal them, VS Codium/Code version.
Tags: #documentation #theme
Creation: 2026-01-15
Update: 2026-06-11
Contributors: [神縁]
---

# Contributing to tokyo-terminal-codium

-----------------------------------------
## Contributors
-----------------------------------------

神縁 - "O captain my captain"

-----------------------------------------
## Access Rules
-----------------------------------------

- **Main branch**: Protected. All changes require a PR.
- **Collaborators**: Contact [@TSLST](https://github.com/TSLST) for write access.
- **Issues/PRs**: Open to all (public repos).

-----------------------------------------
## Workflow
-----------------------------------------

1. Fork the repo (or create a branch).
2. Test changes locally.
3. Open a PR with a clear description.
